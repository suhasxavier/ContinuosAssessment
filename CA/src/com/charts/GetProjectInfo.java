...
public class GetProjectInfo 
{
	...
	public ArrayList getData(String inpProjectName)
	{
		...
		int totest_count=0;
		int todo_count=0;
		double inprogress_hours=0.0;
		double totest_hours=0.0;
		...
		ArrayList inprogresshours = new ArrayList();
		inprogresshours.add("\"Remaining hours for In Progress tasks\"");
		
		ArrayList totesthours = new ArrayList();
		totesthours.add("\"Remaining hours for To Test tasks\"");
		...
		String filedir=prop.getProperty("scrumwise_files");
		...
		for (File f: files)
		{
			...
			for (int i=0;i<all_projects.length();i++)
			{
				...
				System.out.println("inp name is" +inpProjectName);
				...
				{	
					...
					for (int j=0;j<bckItems.length();j++)
						{
							...
							//	System.out.println("this count is imp "+done_count);
								...
								if (task_status.equalsIgnoreCase("To test"))
								{
									...
									double remainingWork=task_obj.getDouble("remainingWork");
									totest_hours=totest_hours+remainingWork;
								}
								if (task_status.equalsIgnoreCase("In progress"))
								{
									inprogress_count=inprogress_count+1;
									double remainingWork=task_obj.getDouble("remainingWork");
									inprogress_hours=inprogress_hours+remainingWork;
								}
					...
					inprogresshours.add(inprogress_hours);
					totesthours.add(totest_hours);
					done_count=0;
					todo_count=0;
					totest_count=0;
					inprogress_count=0;	
		...
		/*
		allResult.add(inprogresshours);
		allResult.add(totesthours);*/
